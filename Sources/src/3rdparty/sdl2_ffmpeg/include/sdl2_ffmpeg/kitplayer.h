...
typedef enum Kit_RendererType {
    KIT_NO_RENDERER = -1,
    KIT_DEFAULT_RENDERER = 0,
    KIT_SDL_RENDERER = 1,
    KIT_OPENGL_RENDERER  = 2,
    KIT_D3D_RENDERER = 3,
    KIT_OPENCV_RENDERER = 3
} Kit_RendererType;
 ...
 */
KIT_API int Kit_GetPlayerVideoSDLData(Kit_Player *player, SDL_Texture *texture);

/**
 * @brief Fetches a new video frame from the player
 * 
 * This functions tries to use, bind and update OpenGL texture without the need of the (sometimes 
 * problematic) SDL renderer
 *
 * Note that the output gltexture cannot be null.
 * 
 * VERIFY It is important to select the correct texture format and size. If you pick a different
 * VERIFY texture format or size from what the decoder outputs, then the decoder will attempt to convert
 * VERIfY the frames to fit the texture. This will slow down the decoder a *lot* however, so if possible,
 * VERIFY pick the texture format from what Kit_GetPlayerInfo() outputs.
 * 
 *  FIXME : add OpenGL recommendations
 * 
 * This function will do nothing if player playback has not been started.
 * 
 * @param player Player instance
 * @param gltexture A previously non zero unsigned int, defining an OpenGL texture 2D
 * @return 0 on success, 1 on error
 */


KIT_API int Kit_GetPlayerVideoOpenGLData(Kit_Player *dec, unsigned int gltexture);

// FIXME : write documentation

KIT_API int Kit_GetPlayerVideoOpenCVData(Kit_Player *dec, uint8_t * pData);

KIT_API int Kit_GetPlayerVideoOpenCVData2(Kit_Player *dec, uint8_t * pYUVData);

KIT_API int Kit_GetPlayerVideoYUV2OpenGLData(Kit_Player *dec, uint8_t * pData);
