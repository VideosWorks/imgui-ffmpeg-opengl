...
int main(int argc, char *argv[]) {
    ...
    // Kit_source src is a struct 
    // its only 2 members are :
    // src->format_ctx (is an ffmpeg context)
    // and
    // src->avio_ctx (is an ffmpeg avio context)
    ...
    // (Kit_Player *) player is a pointer to a structure, and it contains :
    ...
    // FIXME : peek Kit_OutputFormat, containing SDL_PixelFormat (if video/subtitle)
    // probably : pinfo.video.output.format ??
    ...
    while(run) {
        ...
        Kit_GetPlayerVideoSDLData(player, video_tex);
