/* Copyright : Eric Bachard  2018 August 16th */
...
int main(int argc, char *argv[]) {
    ...
    cv::Mat cvMat = cv::Mat(h, w, CV_8UC3);
    cv::Mat const & rMat = cvMat;
    uint8_t * pData = (uint8_t *)rMat.data;

    // NEXT STEP : https://gist.github.com/yohhoy/52b31522dbb751e5296e
    ...
    cv::namedWindow("MyVideo", CV_WINDOW_NORMAL);

    // costly. Causing around 10% of proc load !
    cv::resizeWindow("MyVideo", 1280, 720);
    ...
    while(run) {
        ...
        // OpenCV test
        Kit_GetPlayerVideoOpenCVData(player, pData);
