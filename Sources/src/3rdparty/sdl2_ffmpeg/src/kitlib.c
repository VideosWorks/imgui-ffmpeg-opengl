...
#include "sdl2_ffmpeg/sdl2_ffmpeg_version.h"
...
void Kit_GetVersion(Kit_Version *version) {
    assert(version != NULL);
    version->major = SDL2_FFMPEG_VERSION_MAJOR;
    version->minor = SDL2_FFMPEG_VERSION_MINOR;
    version->patch = SDL2_FFMPEG_VERSION_PATCH;
}
